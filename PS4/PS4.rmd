...
iv.)
The coefficient of education with the endogenous measure $highqua$ is 0.077 wheras with the IV regression it decreases to 0.055. With the reasoning above, the coefficient on education of the IV is expected to increase as it should remove (at least partly) negative the measurement error but not the positive error due to the omitted $ability$. However, the coefficient changes in the other direction. The reason for this might be that that the measurement error in $twihigh$ could even be larger than in $highqua$. However, if we compare the same coefficients in the author's table, we see that the coefficient moves in the direction we expected. The coefficient however should still be positively biased by the omitted variable $ability$.
...
```{r, message=FALSE, warning=FALSE, results='asis'}
Model3 = dlnearn ~ dhigh + 0
Model4 = dlnearn ~ dtwihigh + 0
Model5 = dlnearn ~ dhigh
Model6 = dlnearn ~ dtwihigh

lm3 <- lm(Model3, data = BCHHS_wide, x=TRUE)
lm4 <- lm(Model4, data = BCHHS_wide, x=TRUE)
lm5 <- lm(Model5, data = BCHHS_wide, x=TRUE)
lm6 <- lm(Model6, data = BCHHS_wide, x=TRUE)
```
```{r tab3, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
stargazer(lm3, lm4, lm5, lm6, title="Regression Results Earnings within twins with and without IV", type="latex", header = FALSE,
...
i.)
Because the difference in age measured in years between two twins is always zero (the current record age difference between twins is 85 days, see [here](https://www.nursingschools.net/blog/2010/10/20-amazing-facts-about-twins/)). Therefore, all values for $age$ and $agesq$ would be zero and we would include a variable without any variability.

ii.)
The constant is the average log wage difference if all $\beta$ are set to 0. The constant becomes redudant if we assume that any difference in the dependent variable can only be caused by one of the independent variables. By taking the differences between students, we control for any OVB that is caused by $ability$, as the background and genetic setting from twins is identical. Therefore, we would expect that any difference in wages can only caused by a difference in education and we construct the regression in a way the difference in wage is zero if education is also zero.

The estimated coefficient might have an insignificant effect as almost all of the factors affecting wage should be included by the independent variable education and the fact that differences between twins are used. Since we took the absolute difference in log wage, the constant should also become positive.

Interestingly, the coefficients are more significant than their counterparts in the pooled regression. It seems to be that, after controlling for $ability$ differences, other factors become significant and are captured in the constant. As expected, the coefficients become positive.
...
This directly adresses the endogeneity concern of the OVB and as expected the coefficient on education increases from 0.077 to 0.122 (in IV from 0.055 to 0.123) which confirms the expectation of a positive bias due to OVB.

We also note that using IV now removes the negative bias of measurement error since the coefficient of education increases in the regression with the instrumental variable.

iv.)
Combining the within-twins and IV approach, both sources of bias (measurement error and OVB) should be removed. By using such an approach, one should receive the causal relationship between education and $dlnearn$ and the estimate can be interpreted as the return of education to earnings. Another advantage comes by comparing the OLS and IV regression; the difference between the education coefficients hints at the size of the measurement error bias.

In our setting, the IV approach did not have the intended effect, it could not remove the negative bias due to measurement error in a pooled regression. In the differences-model, using an IV increases the coefficient on education only marginally, so we still assume that a measurement error is present.
...
```{r, message=FALSE, warning=FALSE, results='asis'}
Model7 = dlnearn ~ dhigh + 0
Model8 = dlnearn ~ dtwihigh + 0

lm7 <- lm(Model3, data = BCHHS_wide_no_outliers, x=TRUE)
lm8 <- lm(Model4, data = BCHHS_wide_no_outliers, x=TRUE)
```
```{r tab4, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
stargazer(lm7, lm8, title="Regression Results Earnings without outliers", type="latex", header = FALSE,
 align=TRUE, dep.var.labels=c("Log Wage Difference"),
...
i.)
The coefficients decrease from 0.039 to 0.028 (OLS) and from -0.053 to -0.025, hence the magnitude decreases. Also, while the estimates were significant at the 10 (OLS) and 5 (IV) percent level, both OLS and IV are now not significant at the 10 percent level.
...
ii.)
The interpretation of the coefficients does not change, a 1-year increase in education difference increases wages by approximately 2.8% (OLS). However, the coefficient has become insignificant and can no longer be interpreted as a casual relationship between earnings and education. It seems that most of the variance in education is driven by outliers with a large wage difference, after removing them the reduced variance causes the standard error to increase, resulting in an insignificant coefficient.

iii.)
Dropping the outliers decreases the variance in $dlnearn$. Many twins might already be earning a similar wage. Therefore, outliers are variable observations which add to the variance in earnings differences between twins. If such differences are driven by a difference in education, they confirm the causal effect which the authors want to prove.
...
```{r}
>>>>>>> 240d130582d14c0e820434b313f191a352eae754
Gasoline <- fread('Gasoline.csv')

...
```
  ...
  *a) Let $N = 1$ and $R = 200$. Calculate $\bar{x}^r$ for $r = 1, ...,200$ show them in a histogram. Also calculate the across-replication average, $x$, and sample variance, $s_{barx}$.*
```{r}
...
```
  ...
  *b) Let $N = 5$ and $R = 200$. Calculate $\bar{x}^r$ for $r = 1, ...,200$ show them in a histogram. Also calculate the across-replication average, $x$, and sample variance, $s_{barx}$.*
```{r}
...
```
  ...
  *c) Let $N = 20$ and $R = 200$. Calculate $\bar{x}^r$ for $r = 1, ...,200$ show them in a histogram. Also calculate the across-replication average, $x$, and sample variance, $s_{barx}$.*
```{r}
...
```
  ...
  *d) Let $N = 1000$ and $R = 200$. Calculate $\bar{x}^r$ for $r = 1, ...,200$ show them in a histogram. Also calculate the across-replication average, $x$, and sample variance, $s_{barx}$.*
```{r}
...
The table above shows the convergence of $s_{\bar{x}}$ towards 0 as $N$ increases. The underlying effect is the same as above: extreme values carry less weight, ${\bar{x}}^r$ are closer to each other, variance is reduced.
...
```{r}
beta_1 <- sum((smoke$educ-mean(smoke$educ))*(smoke$cigs-mean(smoke$cigs)))/sum((smoke$educ-mean(smoke$educ))^2)
print(beta_1)
...
cigs_hat_no_constant <- fitted(my_lm_no_constant)#calculates the fitted y values, does same as predict.
...
We can see that the relationship between $educ$ and $cigs$ changes from negative to positive, indicating that more years of education increase cigarette consumption. By removing the constant, we would assume that the regression line goes through the origin, which is clearly not the case. Therefore it is important to include a constant.
  ...
  *d) Now regress $cigs$ on $educ, age, age^2, white$ and $restaurant$ and assume again thatAssumption 2 (Mean-zero error) is satisfied.*
...
```{r}
cor(smoke$age,smoke$residi)
```
According to Assumption 4, the correlation should be 0. As the correlation is really small, we can be confident that this assumption is fulfilled.

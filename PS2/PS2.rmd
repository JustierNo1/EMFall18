...
$Y= X\beta+Z\gamma+\upsilon$ gives residuals $u$ and $R^2_2$
...
\begin{align*}
 u &=y-(X\hat{\beta}+ Z\hat{\gamma}) \\ 
    &= y- X(X'X)^{-1} X'y - Z(Z'Z)^{-1} Z'y \\ 
    &= y - P_X y - P_Z y  \\
    &= e - P_Z y
\end{align*}
Given that:  $$X(X'X)^{-1} X = P_X$$ and $$ Z(Z'Z)^{-1} Z' = P_Z $$
...
stargazer(lm8, title="Regression Results", type="latex", header = FALSE,
 align=TRUE, dep.var.labels=c("Log Wage"),
...
\begin{align*}
&H_0: \beta_4,\space\beta_5\space \textrm{and}\space \beta_6 = 0\space \\
&H_1: \beta_4 or \beta_5 or \beta_6 \neq 0
\end{align*}
...
stargazer(lm8,lm9,lm10, title="Regression Results", type="latex", header = FALSE,
 align=TRUE, dep.var.labels=c("Log Wage"),
 covariate.labels=c("Education","Age","Female", "Female x Education", "Female x Age"),
...
stargazer(lm16, title="Regression Results", type="latex", header = FALSE,
 align=TRUE, dep.var.labels=c("Log Wage"),
...
```
  ...
  *ii) Regress log wages on educ, age, childrenly, and occupation dummies (exclude the dummy for “other”). Following the “p-value” path, test whether the gender wage gap is the same in business and science (i.e. test* $\beta_{business} = \beta{science}$). *Do the test both in R/Stata and “by hand”. How does your answer compare to Stata’s/R’s?*
```{r, message=FALSE, warning=FALSE, results='asis'}
...
stargazer(lm18, title="Regression Results", type="latex", header = FALSE,
 align=TRUE, dep.var.labels=c("Log Wage"),
 covariate.labels=c("Education","Age","Children", "Healthcare", "Technology", "Business", "Science"),
 no.space=TRUE, p.auto = TRUE)
```

Test $\beta_{business} = \beta{science}$ in R:
```{r}
...
$$\frac{\hat{\beta}_{Business}-\hat{\beta}_{Science}-0}{Var(\hat{\beta}_{Business})+Var(\hat{\beta}_{Science})-2*Cov(\hat{\beta}_{Business},\hat{\beta}_{Business})}$$
...
The assumption 2 is pretty a strong assumption, and when it is violeted we say that (at least one of the) variables is endogenous and that the $\hat{\beta}$ is biased. The main sources of this bias are: omitted variable and/or measurment error. The first one could be due to the fact the we don't have a variable for location and the second given by the fact that people self report the wages and thus they are introducing noise in the model. 
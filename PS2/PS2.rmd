...
Females earn on average 44.25% less than males with the same education and age. This is economically significant, by using the average wage and multiplying with the female coefficient we get `r round(mean(USCPS$wage)*0.4425,2)`, a substantial difference.
...
With $H_1: \beta_{children}<0$, $H_0: \beta_{children}>0$. In a one-sided test critical value is 1.64, $\tilde{t}<1.64$ cause us to reject $H_0$. By looking at the reported t-statistics, we can find that we can reject $H_0$ in all occupations other than business". We can confirm $H_1$ in technology, but it cannot be confirmed in all occupations.

...
stargazer(lm1, lm2, lm3, lm4, title="Regression Results Empirical Exercise", type="text", header = FALSE,
...
By using the derivation of slide 86 in the lecture notes (topic 2a), we find that in such a case:
$$\hat\beta=\beta*\frac{\sigma^2_{x^*}}{\sigma^2_{x^*}+\sigma^2_{\eta}}=\lambda\beta$$
Where $x^*$ is the true variable (i.e. $mortalityun$) and $\eta$ is the measurement error. We assume $\beta$ to be positive with the reasoning that higher corruption leads to less efficient health care with child mortality in turn. Therefore, $\hat\beta$ is dragged towards zero by the measurement error and results in a negative bias on our estimator. The higher the measurement error, the larger our negative bias.

We also expect the variance of the coefficient to go up since $V(x)=\sigma^2_{x^*}+\sigma^2_{\eta}$ (by slide 85 of topic 2a).

### ii)
We compare column (1) to column (2) in Table 1 and see that our expectation has been confirmed: the coefficient of $Corruption UN$ has decreased from $0.626$ to $0.528$. Second, we also note that the variance has gone up from $0.083^2$ to $0.091^2$, so our expectations were fully confirmed.
**Explain??? Explanation given above?**
...
ggplot(data=indicators) +
  ...
  labs(title = "Scatter Plot with true x and x with measurement error", x= "Corruption Index UN", y="Child Mortality") +

...
To test whether $\hat\beta>0$, we can perform a one side test with $H_0: \hat\beta <0$ and $H_1: \hat\beta \geq0$. Given the t-value of `r summary(lm1)$coefficients[2,3]` and 89 degrees of freedom, we can look up the probability to find a t-value higher than `r summary(lm1)$coefficients[2,3]` in a percentage t-disribution table. In the table, we find that this probability is equal to 0 (i.e. the p-value is 0). Hence the likelihood that we fail to reject the null hypothesis 0% and we can reject that $\hat\beta<0$.
### ii)
